...
SELECT
  gen_random_uuid(),
  NULL,  -- Fallback for any grade not specifically configured
  5000,  -- Monthly tuition (ETB)
  2000,  -- Registration fee (ETB)
  1200,  -- Bus fee (ETB) - default transport fee
  5,     -- Penalty rate (%)
  '2026',
  b.id,
  NOW()
FROM branches b
WHERE NOT EXISTS (
  SELECT 1 FROM financial_policies fp 
  WHERE fp.branch_id = b.id AND fp.academic_year = '2026' AND fp.grade_level IS NULL
)
...
) AS grades(grade_level, monthly_tuition, registration_fee, bus_fee)
WHERE NOT EXISTS (
  SELECT 1 FROM financial_policies fp
...
SELECT COUNT(*) as total_policies, branch_id, academic_year 
...
ORDER BY b.name, COALESCE(fp.grade_level, 'zzz')
LIMIT 20;
