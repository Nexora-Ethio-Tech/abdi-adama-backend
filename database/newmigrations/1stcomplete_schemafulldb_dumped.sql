...
--
DO $$ BEGIN CREATE TYPE public.audit_category AS ENUM ('Fees', 'Staff');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
DO $$ BEGIN CREATE TYPE public.audit_direction AS ENUM ('In', 'Out');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
DO $$ BEGIN CREATE TYPE public.chat_sender_role AS ENUM ('parent', 'clinic');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
DO $$ BEGIN CREATE TYPE public.fee_status AS ENUM ('standard', 'reduced');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
DO $$ BEGIN CREATE TYPE public.finance_direction AS ENUM ('Income', 'Expense');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
DO $$ BEGIN CREATE TYPE public.risk_level AS ENUM ('Low', 'Medium', 'High');
EXCEPTION
WHEN duplicate_object THEN null;
END $$;
--
...
--
CREATE OR REPLACE FUNCTION public.format_ethiopian_phone(phone text) RETURNS text LANGUAGE plpgsql AS $$ BEGIN -- Remove any existing +251 prefix
...
--
CREATE OR REPLACE FUNCTION public.sort_students_in_section() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN -- This is handled in application logic
    RETURN NEW;
END;
$$;
--
...
CREATE TABLE IF NOT EXISTS public.communication_logs (
    ...
    teacher_note text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT communication_logs_rating_conduct_check CHECK (
        (
            (rating_conduct >= 0)
            AND (rating_conduct <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_homework_check CHECK (
        (
            (rating_homework >= 0)
            AND (rating_homework <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_materials_check CHECK (
        (
            (rating_materials >= 0)
            AND (rating_materials <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_note_taking_check CHECK (
        (
            (rating_note_taking >= 0)
            AND (rating_note_taking <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_participation_check CHECK (
        (
            (rating_participation >= 0)
            AND (rating_participation <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_punctuality_check CHECK (
        (
            (rating_punctuality >= 0)
            AND (rating_punctuality <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_social_check CHECK (
        (
            (rating_social >= 0)
            AND (rating_social <= 3)
        )
    ),
    CONSTRAINT communication_logs_rating_uniform_check CHECK (
        (
            (rating_uniform >= 0)
            AND (rating_uniform <= 3)
        )
    )
);
--
...
CREATE TABLE IF NOT EXISTS public.course_frequency (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT course_frequency_sessions_per_week_check CHECK (
        (
            (sessions_per_week >= 1)
            AND (sessions_per_week <= 10)
        )
    )
);
--
...
CREATE TABLE IF NOT EXISTS public.employee_attendance (
    ...
    recorded_by uuid,
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT employee_attendance_status_check CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY ['present'::character varying, 'absent'::character varying, 'late'::character varying, 'excused'::character varying, 'leave'::character varying]
                )::text []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS public.loans (
    ...
    CONSTRAINT loans_status_check CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY ['pending'::character varying, 'approved'::character varying, 'active'::character varying, 'completed'::character varying, 'rejected'::character varying, 'cancelled'::character varying]
                )::text []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS public.monthly_profit_targets (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    branch_id uuid,
    CONSTRAINT monthly_profit_targets_ethiopian_month_check CHECK (
        (
            (ethiopian_month >= 1)
            AND (ethiopian_month <= 13)
        )
    )
);
--
...
CREATE TABLE IF NOT EXISTS public.payroll_runs (
    ...
    CONSTRAINT payroll_runs_status_check CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY ['draft'::character varying, 'finalized'::character varying, 'exported'::character varying]
                )::text []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS public.schedule_config (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT schedule_config_max_consecutive_periods_check CHECK (
        (
            (max_consecutive_periods >= 1)
            AND (max_consecutive_periods <= 6)
        )
    ),
    CONSTRAINT schedule_config_periods_per_day_check CHECK (
        (
            (periods_per_day >= 3)
            AND (periods_per_day <= 12)
        )
    )
);
--
...
CREATE TABLE IF NOT EXISTS public.silo_communication_book (
    ...
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT silo_communication_book_conduct_check CHECK (
        (
            (conduct >= 1)
            AND (conduct <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_homework_check CHECK (
        (
            (homework >= 1)
            AND (homework <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_materials_check CHECK (
        (
            (materials >= 1)
            AND (materials <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_note_taking_check CHECK (
        (
            (note_taking >= 1)
            AND (note_taking <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_participation_check CHECK (
        (
            (participation >= 1)
            AND (participation <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_punctuality_check CHECK (
        (
            (punctuality >= 1)
            AND (punctuality <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_social_check CHECK (
        (
            (social >= 1)
            AND (social <= 4)
        )
    ),
    CONSTRAINT silo_communication_book_uniform_check CHECK (
        (
            (uniform >= 1)
            AND (uniform <= 4)
        )
...
CREATE TABLE IF NOT EXISTS public.silo_schedule (
    ...
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT silo_schedule_day_of_week_check CHECK (
        (
            (day_of_week >= 0)
            AND (day_of_week <= 6)
        )
...
CREATE TABLE IF NOT EXISTS public.silo_student_grades (
    ...
    total numeric(6, 2) GENERATED ALWAYS AS (
        (
            (
                (
                    COALESCE(quiz_10, (0)::numeric) + COALESCE(assignment_10, (0)::numeric)
                ) + COALESCE(mid_30, (0)::numeric)
            ) + COALESCE(final_50, (0)::numeric)
        )
    ) STORED
);
--
...
CREATE TABLE IF NOT EXISTS public.silo_teacher_rewards (
    ...
    created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT silo_teacher_rewards_reward_month_check CHECK (
        (
            (reward_month >= 1)
            AND (reward_month <= 12)
        )
...
CREATE TABLE IF NOT EXISTS public.staff_notifications (
    ...
    CONSTRAINT staff_notifications_type_check CHECK (
        (
            (type)::text = ANY (
                (
                    ARRAY ['loan'::character varying, 'payroll'::character varying, 'system'::character varying]
                )::text []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS public.teacher_unavailability (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT teacher_unavailability_day_of_week_check CHECK (
        (
            (day_of_week)::text = ANY (
                (
                    ARRAY ['Monday'::character varying, 'Tuesday'::character varying, 'Wednesday'::character varying, 'Thursday'::character varying, 'Friday'::character varying]
                )::text []
            )
        )
    ),
...
CREATE TABLE IF NOT EXISTS public.timetable_runs (
    ...
    generated_by uuid,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT timetable_runs_status_check CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY ['pending'::character varying, 'approved'::character varying, 'rejected'::character varying]
                )::text []
            )
        )
    )
...
CREATE TABLE IF NOT EXISTS public.weekly_plans (
    ...
    week_number integer,
    CONSTRAINT weekly_plans_dean_rating_check CHECK (
        (
            (dean_rating >= 1)
            AND (dean_rating <= 5)
        )
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'absence_queue_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_grades_grade_level_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_grades_pkey'
) THEN
ALTER TABLE ONLY public.academic_grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_history_courses_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_history_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_sections_grade_id_section_name_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_sections_pkey'
) THEN
ALTER TABLE ONLY public.academic_sections
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'access_audit_trail_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'asset_adjustments_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'attendance_history_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'attendance_history_student_id_month_year_key'
) THEN
ALTER TABLE ONLY public.attendance_history
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branch_grade_fees_branch_id_grade_level_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branch_grade_fees_pkey'
) THEN
ALTER TABLE ONLY public.branch_grade_fees
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branches_name_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bulk_communication_recipients_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bulk_communications_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'class_teachers_class_id_teacher_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'class_teachers_pkey'
) THEN
ALTER TABLE ONLY public.class_teachers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_chat_messages_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_visits_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'communication_logs_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'communication_logs_student_id_week_ending_key'
) THEN
ALTER TABLE ONLY public.communication_logs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'course_frequency_course_id_academic_year_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'course_frequency_pkey'
) THEN
ALTER TABLE ONLY public.course_frequency
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'credential_logs_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'driver_notifications_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'email_config_audit_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'email_config_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_attendance_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_attendance_user_id_date_key'
) THEN
ALTER TABLE ONLY public.employee_attendance
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_payroll_profiles_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_payroll_profiles_user_id_key'
) THEN
ALTER TABLE ONLY public.employee_payroll_profiles
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'enrollment_queue_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_access_exam_id_user_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_access_pkey'
) THEN
ALTER TABLE ONLY public.exam_access
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_lockdown_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_question_options_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_questions_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_submissions_exam_id_student_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_submissions_pkey'
) THEN
ALTER TABLE ONLY public.exam_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_violations_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_settings_audit_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_settings_key_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_settings_pkey'
) THEN
ALTER TABLE ONLY public.finance_settings
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_summaries_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_transactions_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'financial_policies_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_locks_grade_level_branch_id_academic_year_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_locks_pkey'
) THEN
ALTER TABLE ONLY public.grade_locks
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_submissions_course_id_teacher_id_submission_type_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_submissions_pkey'
) THEN
ALTER TABLE ONLY public.grade_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grading_configs_grade_level_method_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grading_configs_pkey'
) THEN
ALTER TABLE ONLY public.grading_configs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_books_isbn_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_books_pkey'
) THEN
ALTER TABLE ONLY public.library_books
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_loans_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loan_repayments_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'logistics_notices_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'medicine_inventory_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'monthly_profit_targets_branch_period_unique'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'monthly_profit_targets_pkey'
) THEN
ALTER TABLE ONLY public.monthly_profit_targets
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'parent_student_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_items_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_status_logs_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_items_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_runs_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'registration_config_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'registration_exam_config_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_config_branch_id_academic_year_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_config_pkey'
) THEN
ALTER TABLE ONLY public.schedule_config
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_structure_branch_id_academic_year_class_id_teacher_key'
) THEN
ALTER TABLE ONLY public.schedule_structure
ADD CONSTRAINT schedule_structure_branch_id_academic_year_class_id_teacher_key UNIQUE (
        branch_id,
        academic_year,
        class_id,
        teacher_id,
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_structure_pkey'
) THEN
ALTER TABLE ONLY public.schedule_structure
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'school_config_key_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'school_config_pkey'
) THEN
ALTER TABLE ONLY public.school_config
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'section_assignment_audit_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_announcements_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_books_isbn_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_clinic_chat_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_clinic_records_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_communication_book_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_courses_code_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_courses_pkey'
) THEN
ALTER TABLE ONLY public.silo_courses
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_deadlines_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_drivers_license_number_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_drivers_pkey'
) THEN
ALTER TABLE ONLY public.silo_drivers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_enrollments_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_family_links_parent_user_id_student_identity_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_family_links_pkey'
) THEN
ALTER TABLE ONLY public.silo_family_links
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_identities_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_identities_school_id_key'
) THEN
ALTER TABLE ONLY public.silo_identities
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_logistics_notices_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_parents_email_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_parents_pkey'
) THEN
ALTER TABLE ONLY public.silo_parents
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_route_manifest_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_route_manifest_route_id_student_id_key'
) THEN
ALTER TABLE ONLY public.silo_route_manifest
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_routes_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_schedule_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_sections_name_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_sections_pkey'
) THEN
ALTER TABLE ONLY public.silo_sections
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_grades_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_parents_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_stats_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_students_email_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_students_pkey'
) THEN
ALTER TABLE ONLY public.silo_students
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_teacher_rewards_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'staff_notifications_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aid_usages_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aids_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_collections_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_routes_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'subjects_code_branch_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_department_heads_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_department_heads_teacher_id_department_name_key'
) THEN
ALTER TABLE ONLY public.teacher_department_heads
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_exam_assignments_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_of_week_votes_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_of_week_votes_student_id_cycle_key_key'
) THEN
ALTER TABLE ONLY public.teacher_of_week_votes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_unavailability_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_unavailability_teacher_id_day_of_week_period_number_key'
) THEN
ALTER TABLE ONLY public.teacher_unavailability
ADD CONSTRAINT teacher_unavailability_teacher_id_day_of_week_period_number_key UNIQUE (
        teacher_id,
        day_of_week,
        period_number,
        academic_year
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'timetable_runs_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'uq_identity_role'
) THEN
ALTER TABLE ONLY public.silo_users
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'uq_teacher_reward'
) THEN
ALTER TABLE ONLY public.silo_teacher_rewards
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'users_digital_id_key'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'users_zk_device_id_key'
) THEN
ALTER TABLE ONLY public.users
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'weekly_plans_pkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_academic_sections_branch'
) THEN CREATE INDEX idx_academic_sections_branch ON public.academic_sections USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_academic_sections_grade'
) THEN CREATE INDEX idx_academic_sections_grade ON public.academic_sections USING btree (grade_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_asset_adjustments_asset'
) THEN CREATE INDEX idx_asset_adjustments_asset ON public.asset_adjustments USING btree (asset_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_asset_adjustments_branch'
) THEN CREATE INDEX idx_asset_adjustments_branch ON public.asset_adjustments USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_audit_category'
) THEN CREATE INDEX idx_audit_category ON public.audit_log USING btree (category);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_audit_direction'
) THEN CREATE INDEX idx_audit_direction ON public.audit_log USING btree (direction);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_audit_timestamp'
) THEN CREATE INDEX idx_audit_timestamp ON public.audit_log USING btree ("timestamp");
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_branch_grade_fees_branch'
) THEN CREATE INDEX idx_branch_grade_fees_branch ON public.branch_grade_fees USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_clinic_chat_student'
) THEN CREATE INDEX idx_clinic_chat_student ON public.silo_clinic_chat USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_comm_book_id'
) THEN CREATE INDEX idx_comm_book_id ON public.silo_communication_book USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_course_freq_branch'
) THEN CREATE INDEX idx_course_freq_branch ON public.course_frequency USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_course_freq_course'
) THEN CREATE INDEX idx_course_freq_course ON public.course_frequency USING btree (course_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_driver_notifications_created_at'
) THEN CREATE INDEX idx_driver_notifications_created_at ON public.driver_notifications USING btree (created_at);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_driver_notifications_driver_id'
) THEN CREATE INDEX idx_driver_notifications_driver_id ON public.driver_notifications USING btree (driver_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_employee_attendance_user_date'
) THEN CREATE INDEX idx_employee_attendance_user_date ON public.employee_attendance USING btree (user_id, date);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_employee_payroll_profiles_user_id'
) THEN CREATE INDEX idx_employee_payroll_profiles_user_id ON public.employee_payroll_profiles USING btree (user_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_family_links_parent'
) THEN CREATE INDEX idx_family_links_parent ON public.silo_family_links USING btree (parent_user_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_branch'
) THEN CREATE INDEX idx_finance_transactions_branch ON public.finance_transactions USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_branch_date'
) THEN CREATE INDEX idx_finance_transactions_branch_date ON public.finance_transactions USING btree (branch_id, date);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_date'
) THEN CREATE INDEX idx_finance_transactions_date ON public.finance_transactions USING btree (date);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_eth'
) THEN CREATE INDEX idx_finance_transactions_eth ON public.finance_transactions USING btree (ethiopic_month, ethiopic_year);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_student'
) THEN CREATE INDEX idx_finance_transactions_student ON public.finance_transactions USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_student_id'
) THEN CREATE INDEX idx_finance_transactions_student_id ON public.finance_transactions USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_finance_transactions_type'
) THEN CREATE INDEX idx_finance_transactions_type ON public.finance_transactions USING btree (type);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_grade_locks_branch'
) THEN CREATE INDEX idx_grade_locks_branch ON public.grade_locks USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_loan_repayments_loan_id'
) THEN CREATE INDEX idx_loan_repayments_loan_id ON public.loan_repayments USING btree (loan_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_loans_employee_id'
) THEN CREATE INDEX idx_loans_employee_id ON public.loans USING btree (employee_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_monthly_profit_targets_branch'
) THEN CREATE INDEX idx_monthly_profit_targets_branch ON public.monthly_profit_targets USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_monthly_profit_targets_branch_period'
) THEN CREATE UNIQUE INDEX idx_monthly_profit_targets_branch_period ON public.monthly_profit_targets USING btree (branch_id, ethiopian_month, target_year)
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_payment_items_fee_type'
) THEN CREATE INDEX idx_payment_items_fee_type ON public.payment_items USING btree (fee_type);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_payments_student_month'
) THEN CREATE INDEX idx_payments_student_month ON public.payments USING btree (student_id, month);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_payroll_items_employee_id'
) THEN CREATE INDEX idx_payroll_items_employee_id ON public.payroll_items USING btree (employee_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_payroll_items_run_id'
) THEN CREATE INDEX idx_payroll_items_run_id ON public.payroll_items USING btree (payroll_run_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_payroll_runs_month_year'
) THEN CREATE INDEX idx_payroll_runs_month_year ON public.payroll_runs USING btree (month, year);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_schedule_config_branch'
) THEN CREATE INDEX idx_schedule_config_branch ON public.schedule_config USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_schedule_structure_branch'
) THEN CREATE INDEX idx_schedule_structure_branch ON public.schedule_structure USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_schedule_structure_year'
) THEN CREATE INDEX idx_schedule_structure_year ON public.schedule_structure USING btree (academic_year);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_section_assignment_audit_created'
) THEN CREATE INDEX idx_section_assignment_audit_created ON public.section_assignment_audit USING btree (created_at);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_section_assignment_audit_student'
) THEN CREATE INDEX idx_section_assignment_audit_student ON public.section_assignment_audit USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_books_status'
) THEN CREATE INDEX idx_silo_books_status ON public.silo_books USING btree (status);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_courses_teacher'
) THEN CREATE INDEX idx_silo_courses_teacher ON public.silo_courses USING btree (teacher_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_deadlines_due_date'
) THEN CREATE INDEX idx_silo_deadlines_due_date ON public.silo_deadlines USING btree (due_date);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_deadlines_section'
) THEN CREATE INDEX idx_silo_deadlines_section ON public.silo_deadlines USING btree (section_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_enrollments_course'
) THEN CREATE INDEX idx_silo_enrollments_course ON public.silo_enrollments USING btree (course_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_enrollments_student'
) THEN CREATE INDEX idx_silo_enrollments_student ON public.silo_enrollments USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_identities_school_id'
) THEN CREATE INDEX idx_silo_identities_school_id ON public.silo_identities USING btree (school_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_loans_student_id'
) THEN CREATE INDEX idx_silo_loans_student_id ON public.silo_loans USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_logistics_notices_time'
) THEN CREATE INDEX idx_silo_logistics_notices_time ON public.silo_logistics_notices USING btree ("timestamp");
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_parents_email'
) THEN CREATE INDEX idx_silo_parents_email ON public.silo_parents USING btree (email);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_route_manifest_route_id'
) THEN CREATE INDEX idx_silo_route_manifest_route_id ON public.silo_route_manifest USING btree (route_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_routes_driver_id'
) THEN CREATE INDEX idx_silo_routes_driver_id ON public.silo_routes USING btree (driver_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_schedule_day'
) THEN CREATE INDEX idx_silo_schedule_day ON public.silo_schedule USING btree (day_of_week);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_schedule_section'
) THEN CREATE INDEX idx_silo_schedule_section ON public.silo_schedule USING btree (section_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_students_email'
) THEN CREATE INDEX idx_silo_students_email ON public.silo_students USING btree (email);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_silo_users_identity_id'
) THEN CREATE INDEX idx_silo_users_identity_id ON public.silo_users USING btree (identity_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_staff_notifications_is_read'
) THEN CREATE INDEX idx_staff_notifications_is_read ON public.staff_notifications USING btree (is_read);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_staff_notifications_user_id'
) THEN CREATE INDEX idx_staff_notifications_user_id ON public.staff_notifications USING btree (user_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_student_aid_usages_student_month'
) THEN CREATE INDEX idx_student_aid_usages_student_month ON public.student_aid_usages USING btree (student_id, month);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_student_aids_student_id'
) THEN CREATE INDEX idx_student_aids_student_id ON public.student_aids USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_student_collections_month'
) THEN CREATE INDEX idx_student_collections_month ON public.student_collections USING btree (month);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_student_grades_id'
) THEN CREATE INDEX idx_student_grades_id ON public.silo_student_grades USING btree (student_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_students_grade'
) THEN CREATE INDEX idx_students_grade ON public.students USING btree (grade);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_students_previous_section'
) THEN CREATE INDEX idx_students_previous_section ON public.students USING btree (previous_section_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_students_section'
) THEN CREATE INDEX idx_students_section ON public.students USING btree (section_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_students_status'
) THEN CREATE INDEX idx_students_status ON public.students USING btree (status);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_teacher_of_week_votes_cycle'
) THEN CREATE INDEX idx_teacher_of_week_votes_cycle ON public.teacher_of_week_votes USING btree (branch_id, cycle_key);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_teacher_unavail_branch'
) THEN CREATE INDEX idx_teacher_unavail_branch ON public.teacher_unavailability USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_teacher_unavail_teacher'
) THEN CREATE INDEX idx_teacher_unavail_teacher ON public.teacher_unavailability USING btree (teacher_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_timetable_runs_branch'
) THEN CREATE INDEX idx_timetable_runs_branch ON public.timetable_runs USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_timetable_runs_status'
) THEN CREATE INDEX idx_timetable_runs_status ON public.timetable_runs USING btree (status);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_users_branch'
) THEN CREATE INDEX idx_users_branch ON public.users USING btree (branch_id);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_users_digital'
) THEN CREATE INDEX idx_users_digital ON public.users USING btree (digital_id);
END IF;
END $$;
--
...
) THEN CREATE INDEX idx_users_email ON public.users USING btree (email);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_users_role'
) THEN CREATE INDEX idx_users_role ON public.users USING btree (role);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'idx_users_username'
) THEN CREATE INDEX idx_users_username ON public.users USING btree (username);
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relname = 'uq_grades_student_course_type_term'
) THEN CREATE UNIQUE INDEX uq_grades_student_course_type_term ON public.grades USING btree (
    student_id,
    course_id,
    type,
    academic_year,
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_academic_grades_updated'
) THEN CREATE TRIGGER trg_academic_grades_updated BEFORE
UPDATE ON public.academic_grades FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_academic_sections_updated'
) THEN CREATE TRIGGER trg_academic_sections_updated BEFORE
UPDATE ON public.academic_sections FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_exams_updated'
) THEN CREATE TRIGGER trg_exams_updated BEFORE
UPDATE ON public.exams FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_inventory_updated'
) THEN CREATE TRIGGER trg_inventory_updated BEFORE
UPDATE ON public.inventory FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_medicine_updated'
) THEN CREATE TRIGGER trg_medicine_updated BEFORE
UPDATE ON public.medicine_inventory FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_pending_apps_upd'
) THEN CREATE TRIGGER trg_pending_apps_upd BEFORE
UPDATE ON public.pending_applications FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_registration_config_updated'
) THEN CREATE TRIGGER trg_registration_config_updated BEFORE
UPDATE ON public.registration_config FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_students_updated'
) THEN CREATE TRIGGER trg_students_updated BEFORE
UPDATE ON public.students FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_teachers_updated'
) THEN CREATE TRIGGER trg_teachers_updated BEFORE
UPDATE ON public.teachers FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
) THEN CREATE TRIGGER trg_users_updated BEFORE
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'trg_weekly_plans_upd'
) THEN CREATE TRIGGER trg_weekly_plans_upd BEFORE
UPDATE ON public.weekly_plans FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'absence_queue_student_id_fkey'
) THEN
ALTER TABLE ONLY public.absence_queue
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_grades_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_history_courses_history_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_history_courses
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_history_student_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_history
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_sections_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_sections
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_sections_grade_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_sections
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_sections_room_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_sections
ADD CONSTRAINT academic_sections_room_teacher_id_fkey FOREIGN KEY (room_teacher_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'academic_years_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.academic_years
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'access_audit_trail_attempted_branch_fkey'
) THEN
ALTER TABLE ONLY public.access_audit_trail
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'access_audit_trail_user_branch_fkey'
) THEN
ALTER TABLE ONLY public.access_audit_trail
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'access_audit_trail_user_id_fkey'
) THEN
ALTER TABLE ONLY public.access_audit_trail
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'asset_adjustments_asset_id_fkey'
) THEN
ALTER TABLE ONLY public.asset_adjustments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'asset_adjustments_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.asset_adjustments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'assets_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.assets
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'attendance_history_student_id_fkey'
) THEN
ALTER TABLE ONLY public.attendance_history
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'audit_log_student_id_fkey'
) THEN
ALTER TABLE ONLY public.audit_log
ADD CONSTRAINT audit_log_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.students(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branch_grade_fees_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.branch_grade_fees
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branch_grade_fees_updated_by_fkey'
) THEN
ALTER TABLE ONLY public.branch_grade_fees
ADD CONSTRAINT branch_grade_fees_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bulk_communication_recipients_application_id_fkey'
) THEN
ALTER TABLE ONLY public.bulk_communication_recipients
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bulk_communication_recipients_communication_id_fkey'
) THEN
ALTER TABLE ONLY public.bulk_communication_recipients
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bulk_communications_sent_by_fkey'
) THEN
ALTER TABLE ONLY public.bulk_communications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'class_teachers_class_id_fkey'
) THEN
ALTER TABLE ONLY public.class_teachers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'class_teachers_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.class_teachers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'classes_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.classes
ADD CONSTRAINT classes_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
ALTER TABLE ONLY public.classes
ADD CONSTRAINT classes_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_chat_messages_sender_id_fkey'
) THEN
ALTER TABLE ONLY public.clinic_chat_messages
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_chat_messages_student_id_fkey'
) THEN
ALTER TABLE ONLY public.clinic_chat_messages
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_visits_logged_by_fkey'
) THEN
ALTER TABLE ONLY public.clinic_visits
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'clinic_visits_student_id_fkey'
) THEN
ALTER TABLE ONLY public.clinic_visits
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'communication_logs_student_id_fkey'
) THEN
ALTER TABLE ONLY public.communication_logs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'communication_logs_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.communication_logs
ADD CONSTRAINT communication_logs_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'course_frequency_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.course_frequency
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'course_frequency_course_id_fkey'
) THEN
ALTER TABLE ONLY public.course_frequency
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'courses_class_id_fkey'
) THEN
ALTER TABLE ONLY public.courses
ADD CONSTRAINT courses_class_id_fkey FOREIGN KEY (class_id) REFERENCES public.classes(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
ALTER TABLE ONLY public.courses
ADD CONSTRAINT courses_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'credential_logs_generated_by_fkey'
) THEN
ALTER TABLE ONLY public.credential_logs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'credential_logs_user_id_fkey'
) THEN
ALTER TABLE ONLY public.credential_logs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'driver_notifications_driver_id_fkey'
) THEN
ALTER TABLE ONLY public.driver_notifications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'email_config_audit_changed_by_fkey'
) THEN
ALTER TABLE ONLY public.email_config_audit
ADD CONSTRAINT email_config_audit_changed_by_fkey FOREIGN KEY (changed_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'email_config_updated_by_fkey'
) THEN
ALTER TABLE ONLY public.email_config
ADD CONSTRAINT email_config_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'emergency_contacts_student_id_fkey'
) THEN
ALTER TABLE ONLY public.emergency_contacts
...
ALTER TABLE ONLY public.employee_attendance
ADD CONSTRAINT employee_attendance_recorded_by_fkey FOREIGN KEY (recorded_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_attendance_user_id_fkey'
) THEN
ALTER TABLE ONLY public.employee_attendance
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_payroll_profiles_user_id_fkey'
) THEN
ALTER TABLE ONLY public.employee_payroll_profiles
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'events_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.events
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_access_exam_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_access
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_access_user_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_access
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_lockdown_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_lockdown
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_question_options_question_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_question_options
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_questions_exam_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_questions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_submissions_exam_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_submissions_student_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exam_violations_submission_id_fkey'
) THEN
ALTER TABLE ONLY public.exam_violations
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exams_course_id_fkey'
) THEN
ALTER TABLE ONLY public.exams
ADD CONSTRAINT exams_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exams_hidden_by_fkey'
) THEN
ALTER TABLE ONLY public.exams
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exams_locked_by_fkey'
) THEN
ALTER TABLE ONLY public.exams
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'exams_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.exams
ADD CONSTRAINT exams_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_settings_audit_changed_by_fkey'
) THEN
ALTER TABLE ONLY public.finance_settings_audit
ADD CONSTRAINT finance_settings_audit_changed_by_fkey FOREIGN KEY (changed_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_settings_updated_by_fkey'
) THEN
ALTER TABLE ONLY public.finance_settings
ADD CONSTRAINT finance_settings_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_transactions_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.finance_transactions
ADD CONSTRAINT finance_transactions_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'finance_transactions_student_id_fkey'
) THEN
ALTER TABLE ONLY public.finance_transactions
ADD CONSTRAINT finance_transactions_student_id_fkey FOREIGN KEY (student_id) REFERENCES public.students(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'financial_policies_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.financial_policies
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_locks_academic_year_id_fkey'
) THEN
ALTER TABLE ONLY public.grade_locks
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_locks_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.grade_locks
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_locks_locked_by_fkey'
) THEN
ALTER TABLE ONLY public.grade_locks
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_submissions_course_id_fkey'
) THEN
ALTER TABLE ONLY public.grade_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grade_submissions_submitted_by_fkey'
) THEN
ALTER TABLE ONLY public.grade_submissions
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grades_course_id_fkey'
) THEN
ALTER TABLE ONLY public.grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grades_student_id_fkey'
) THEN
ALTER TABLE ONLY public.grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'grades_submitted_by_fkey'
) THEN
ALTER TABLE ONLY public.grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_books_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.library_books
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_loans_book_id_fkey'
) THEN
ALTER TABLE ONLY public.library_loans
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'library_loans_student_id_fkey'
) THEN
ALTER TABLE ONLY public.library_loans
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loan_repayments_loan_id_fkey'
) THEN
ALTER TABLE ONLY public.loan_repayments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loan_repayments_payroll_id_fkey'
) THEN
ALTER TABLE ONLY public.loan_repayments
ADD CONSTRAINT loan_repayments_payroll_id_fkey FOREIGN KEY (payroll_id) REFERENCES public.payroll_runs(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loans_audited_by_fkey'
) THEN
ALTER TABLE ONLY public.loans
ADD CONSTRAINT loans_audited_by_fkey FOREIGN KEY (audited_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loans_employee_id_fkey'
) THEN
ALTER TABLE ONLY public.loans
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loans_issued_by_fkey'
) THEN
ALTER TABLE ONLY public.loans
ADD CONSTRAINT loans_issued_by_fkey FOREIGN KEY (issued_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'logistics_notices_driver_id_fkey'
) THEN
ALTER TABLE ONLY public.logistics_notices
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'medicine_inventory_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.medicine_inventory
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'monthly_profit_targets_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.monthly_profit_targets
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'monthly_profit_targets_updated_by_fkey'
) THEN
ALTER TABLE ONLY public.monthly_profit_targets
ADD CONSTRAINT monthly_profit_targets_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'notices_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.notices
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'notices_posted_by_fkey'
) THEN
ALTER TABLE ONLY public.notices
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'parent_student_parent_id_fkey'
) THEN
ALTER TABLE ONLY public.parent_student
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'parent_student_student_id_fkey'
) THEN
ALTER TABLE ONLY public.parent_student
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'parents_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.parents
ADD CONSTRAINT parents_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'parents_linked_student_id_fkey'
) THEN
ALTER TABLE ONLY public.parents
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_items_payment_id_fkey'
) THEN
ALTER TABLE ONLY public.payment_items
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payment_status_logs_student_id_fkey'
) THEN
ALTER TABLE ONLY public.payment_status_logs
...
ALTER TABLE ONLY public.payments
ADD CONSTRAINT payments_payer_id_fkey FOREIGN KEY (payer_id) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_items_employee_id_fkey'
) THEN
ALTER TABLE ONLY public.payroll_items
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_items_payroll_run_id_fkey'
) THEN
ALTER TABLE ONLY public.payroll_items
...
ALTER TABLE ONLY public.payroll_runs
ADD CONSTRAINT payroll_runs_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_runs_finalized_by_fkey'
) THEN
ALTER TABLE ONLY public.payroll_runs
ADD CONSTRAINT payroll_runs_finalized_by_fkey FOREIGN KEY (finalized_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'payroll_runs_generated_by_fkey'
) THEN
ALTER TABLE ONLY public.payroll_runs
ADD CONSTRAINT payroll_runs_generated_by_fkey FOREIGN KEY (generated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
ADD CONSTRAINT pending_applications_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_finance_removed_by_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
ADD CONSTRAINT pending_applications_finance_removed_by_fkey FOREIGN KEY (finance_removed_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_finance_user_id_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
ADD CONSTRAINT pending_applications_finance_user_id_fkey FOREIGN KEY (finance_user_id) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_parent_user_id_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
ADD CONSTRAINT pending_applications_parent_user_id_fkey FOREIGN KEY (parent_user_id) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_payment_confirmed_by_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_reviewed_by_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_section_assigned_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'pending_applications_student_user_id_fkey'
) THEN
ALTER TABLE ONLY public.pending_applications
ADD CONSTRAINT pending_applications_student_user_id_fkey FOREIGN KEY (student_user_id) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'registration_config_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.registration_config
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'registration_exam_config_application_id_fkey'
) THEN
ALTER TABLE ONLY public.registration_exam_config
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'routes_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.routes
...
ALTER TABLE ONLY public.routes
ADD CONSTRAINT routes_driver_id_fkey FOREIGN KEY (driver_id) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'routes_vehicle_id_fkey'
) THEN
ALTER TABLE ONLY public.routes
ADD CONSTRAINT routes_vehicle_id_fkey FOREIGN KEY (vehicle_id) REFERENCES public.vehicles(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_config_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.schedule_config
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_structure_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.schedule_structure
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_structure_class_id_fkey'
) THEN
ALTER TABLE ONLY public.schedule_structure
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedule_structure_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.schedule_structure
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'schedules_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.schedules
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'section_assignment_audit_assigned_by_fkey'
) THEN
ALTER TABLE ONLY public.section_assignment_audit
ADD CONSTRAINT section_assignment_audit_assigned_by_fkey FOREIGN KEY (assigned_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'section_assignment_audit_from_section_id_fkey'
) THEN
ALTER TABLE ONLY public.section_assignment_audit
ADD CONSTRAINT section_assignment_audit_from_section_id_fkey FOREIGN KEY (from_section_id) REFERENCES public.classes(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'section_assignment_audit_student_id_fkey'
) THEN
ALTER TABLE ONLY public.section_assignment_audit
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'section_assignment_audit_to_section_id_fkey'
) THEN
ALTER TABLE ONLY public.section_assignment_audit
ADD CONSTRAINT section_assignment_audit_to_section_id_fkey FOREIGN KEY (to_section_id) REFERENCES public.classes(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_clinic_chat_sender_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_clinic_chat
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_clinic_chat_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_clinic_chat
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_communication_book_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_communication_book
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_courses_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_courses
ADD CONSTRAINT silo_courses_teacher_id_fkey FOREIGN KEY (teacher_id) REFERENCES public.silo_identities(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_deadlines_course_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_deadlines
ADD CONSTRAINT silo_deadlines_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.silo_courses(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_deadlines_section_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_deadlines
ADD CONSTRAINT silo_deadlines_section_id_fkey FOREIGN KEY (section_id) REFERENCES public.silo_sections(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_drivers_user_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_drivers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_enrollments_course_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_enrollments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_enrollments_section_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_enrollments
ADD CONSTRAINT silo_enrollments_section_id_fkey FOREIGN KEY (section_id) REFERENCES public.silo_sections(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_enrollments_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_enrollments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_family_links_parent_user_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_family_links
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_family_links_student_identity_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_family_links
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_loans_book_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_loans
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_logistics_notices_sender_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_logistics_notices
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_parents_user_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_parents
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_route_manifest_route_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_route_manifest
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_route_manifest_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_route_manifest
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_routes_driver_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_routes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_schedule_course_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_schedule
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_schedule_section_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_schedule
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_grades_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_student_grades
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_parents_parent_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_student_parents
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_parents_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_student_parents
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_student_stats_student_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_student_stats
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_students_user_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_students
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_teacher_rewards_teacher_identity_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_teacher_rewards
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'silo_users_identity_id_fkey'
) THEN
ALTER TABLE ONLY public.silo_users
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'staff_notifications_user_id_fkey'
) THEN
ALTER TABLE ONLY public.staff_notifications
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aid_usages_payment_id_fkey'
) THEN
ALTER TABLE ONLY public.student_aid_usages
ADD CONSTRAINT student_aid_usages_payment_id_fkey FOREIGN KEY (payment_id) REFERENCES public.payments(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aid_usages_student_aid_id_fkey'
) THEN
ALTER TABLE ONLY public.student_aid_usages
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aid_usages_student_id_fkey'
) THEN
ALTER TABLE ONLY public.student_aid_usages
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aids_approved_by_fkey'
) THEN
ALTER TABLE ONLY public.student_aids
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aids_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.student_aids
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_aids_student_id_fkey'
) THEN
ALTER TABLE ONLY public.student_aids
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_attendance_recorded_by_fkey'
) THEN
ALTER TABLE ONLY public.student_attendance
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_collections_student_id_fkey'
) THEN
ALTER TABLE ONLY public.student_collections
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_routes_route_id_fkey'
) THEN
ALTER TABLE ONLY public.student_routes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'student_routes_student_id_fkey'
) THEN
ALTER TABLE ONLY public.student_routes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'students_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.students
ADD CONSTRAINT students_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'students_previous_section_id_fkey'
) THEN
ALTER TABLE ONLY public.students
ADD CONSTRAINT students_previous_section_id_fkey FOREIGN KEY (previous_section_id) REFERENCES public.classes(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'students_section_id_fkey'
) THEN
ALTER TABLE ONLY public.students
ADD CONSTRAINT students_section_id_fkey FOREIGN KEY (section_id) REFERENCES public.classes(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'subjects_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.subjects
...
ALTER TABLE ONLY public.system_settings
ADD CONSTRAINT system_settings_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.users(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_department_heads_assigned_by_fkey'
) THEN
ALTER TABLE ONLY public.teacher_department_heads
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_department_heads_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_department_heads
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_exam_assignments_assigned_by_fkey'
) THEN
ALTER TABLE ONLY public.teacher_exam_assignments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_exam_assignments_exam_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_exam_assignments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_exam_assignments_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_exam_assignments
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_of_week_votes_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_of_week_votes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_of_week_votes_student_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_of_week_votes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_of_week_votes_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_of_week_votes
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_unavailability_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_unavailability
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teacher_unavailability_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.teacher_unavailability
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teachers_assigned_room_section_id_fkey'
) THEN
ALTER TABLE ONLY public.teachers
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'teachers_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.teachers
ADD CONSTRAINT teachers_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'timetable_runs_branch_id_fkey'
) THEN
ALTER TABLE ONLY public.timetable_runs
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'timetable_runs_generated_by_fkey'
) THEN
ALTER TABLE ONLY public.timetable_runs
...
ALTER TABLE ONLY public.users
ADD CONSTRAINT users_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'weekly_plans_course_id_fkey'
) THEN
ALTER TABLE ONLY public.weekly_plans
ADD CONSTRAINT weekly_plans_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'weekly_plans_dept_head_id_fkey'
) THEN
ALTER TABLE ONLY public.weekly_plans
ADD CONSTRAINT weekly_plans_dept_head_id_fkey FOREIGN KEY (dept_head_id) REFERENCES public.teachers(id) ON DELETE
SET NULL;
END IF;
END $$;
--
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'weekly_plans_reviewed_by_fkey'
) THEN
ALTER TABLE ONLY public.weekly_plans
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'weekly_plans_teacher_id_fkey'
) THEN
ALTER TABLE ONLY public.weekly_plans
