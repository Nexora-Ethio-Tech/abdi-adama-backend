...
ALTER TABLE IF EXISTS public.grade_submissions
  DROP CONSTRAINT IF EXISTS grade_submissions_course_id_teacher_id_submission_type_key;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_grade_submissions_unique_course_teacher_type_year_sem
  ON public.grade_submissions(course_id, teacher_id, submission_type, academic_year, semester);

-- Non-unique index for fast lookups by period (kept for compatibility)
CREATE INDEX IF NOT EXISTS idx_grade_submissions_by_period
  ON public.grade_submissions(course_id, teacher_id, submission_type, academic_year, semester);
