...
export const switchRole = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('SELECT id, name, email, role, branch_id, status, is_branch_auditor, digital_id FROM users WHERE id = $1', [user.id]);
        if (result.rows.length === 0) {
        ...
        const token = jwt.sign({ id: dbUser.id, email: dbUser.email, role: newRole, branch_id: dbUser.branch_id, status: dbUser.status, is_branch_auditor: dbUser.is_branch_auditor }, JWT_SECRET, { expiresIn: '24h' });
        // Determine redirect
        let dashboard = '/dashboard/school-admin';
        if (newRole === 'super-admin')
            ...
            dashboard = '/dashboard/parent';
        else if (newRole === 'finance-clerk')
            dashboard = '/dashboard/finance';
        else if (newRole === 'vice-principal')
            dashboard = '/dashboard/vice-principal';
