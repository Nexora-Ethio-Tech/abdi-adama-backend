...
ADD COLUMN IF NOT EXISTS peer_reviewed_by UUID REFERENCES teachers(id);
ALTER TABLE weekly_plans
ADD COLUMN IF NOT EXISTS peer_feedback TEXT;
ALTER TABLE weekly_plans
ADD COLUMN IF NOT EXISTS peer_rating SMALLINT CHECK (
        peer_rating BETWEEN 1 AND 5
    );
ALTER TABLE weekly_plans
ADD COLUMN IF NOT EXISTS peer_reviewed_at TIMESTAMPTZ;
...
ADD COLUMN IF NOT EXISTS fee_reduction_reviewed_by UUID REFERENCES users(id);
ALTER TABLE students
ADD COLUMN IF NOT EXISTS fee_reduction_reviewed_at TIMESTAMPTZ;
...
CREATE TRIGGER IF NOT EXISTS trg_academic_years_updated BEFORE
UPDATE ON academic_years FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER IF NOT EXISTS trg_revenue_targets_updated BEFORE
