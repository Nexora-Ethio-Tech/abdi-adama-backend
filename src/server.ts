...
async function bootstrap(): Promise<void> {
  try {
    ...
    // Automatically reconcile any unlinked payment-confirmed applications (auto-heals production db on boot)
    async function reconcileUnlinkedApplications() {
      try {
        ...
        logger.info(`[reconcile] Found ${appRes.rows.length} unlinked payment-confirmed applications. Healing...`);

        // Lazy-require user service to avoid potential circular dependencies
        ...
        for (const app of appRes.rows) {
          ...
          if (!app.parent_user_id) {
            logger.warn(`[reconcile] Skipping application ${app.id} because parent_user_id is missing.`);
            continue;
          }
          ...
          if (app.applicant_email) {
            ...
            if (userRes.rows.length > 0) {
              ...
              logger.info(`[reconcile] Found existing student user account by email: ${studentUserId}`);
            }
          }

          if (!studentUserId) {
            const userResByName = await pool.query(
              "SELECT id FROM users WHERE name = $1 AND role = 'student' AND branch_id = $2 LIMIT 1",
              [app.applicant_name, app.branch_id]
            );
            if (userResByName.rows.length > 0) {
              studentUserId = userResByName.rows[0].id;
              logger.info(`[reconcile] Found existing student user account by name: ${studentUserId}`);
            }
          }
          ...
          if (!studentUserId) {
            ...
            logger.info(`[reconcile] No student account found. Creating new student user with email: ${studentEmail}`);
            ...
            studentUserId = studentCreate.user.id;
          }

          const studentIdRes = await pool.query(
            'SELECT id FROM students WHERE user_id = $1 LIMIT 1',
            [studentUserId]
          );

          if (studentIdRes.rows.length === 0) {
            logger.error(`[reconcile] Failed to locate student profile in students table for user ID ${studentUserId}.`);
            continue;
          }
          const studentId = studentIdRes.rows[0].id;

          const parentIdRes = await pool.query(
            'SELECT id FROM parents WHERE user_id = $1 LIMIT 1',
            [app.parent_user_id]
          );

          if (parentIdRes.rows.length === 0) {
            logger.error(`[reconcile] Failed to locate parent profile in parents table for user ID ${app.parent_user_id}.`);
            continue;
          }
          ...
          logger.info(`[reconcile] Successfully healed and linked student ${app.applicant_name} to parent ${app.parent_user_id}`);
        }
      } catch (err: any) {
        logger.error(`[reconcile] Error during auto-reconciliation: ${err.message}`);
      }
    }
