...
async function seedMissingAccounts() {
    ...
    try {
        console.log('🌱 Seeding missing staff and student accounts...\n');
        ...
        async function seedUser(digital_id: string, email: string, name: string, role: string, password: string) {
            ...
            const existing = await client.query(
                'SELECT id FROM users WHERE digital_id = $1 OR email = $2 LIMIT 1',
                [digital_id, email]
            );
           ...
           WHERE digital_id = $3 OR email = $4`,
                    [email, passwordHash, digital_id, email]
                );
        ...
        for (const student of students) {
            ...
            // Create student profile record - check if exists first
            ...
            if (existingStudent.rows.length > 0) {
                await client.query(
                    'UPDATE students SET grade = $1, status = $2 WHERE user_id = $3',
                    [student.grade, 'Active', userId]
                );
            } else {
                await client.query(
                    'INSERT INTO students (user_id, branch_id, grade, status) VALUES ($1, $2, $3, $4)',
                    [userId, branchId, student.grade, 'Active']
                );
            }
        }
        console.log('');
