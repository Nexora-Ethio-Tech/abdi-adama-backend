...
SET grade = (
    CASE
      ...
      WHEN name ILIKE 'Grade %' THEN SUBSTRING(name, 7, 2)::VARCHAR(10) -- Try to extract 1-2 digit number from start
      WHEN name ~ '^\d{1,2}' THEN (
        SUBSTRING(
          name
          FROM '^\d{1,2}'
        )
      )::VARCHAR(10) -- Fallback: use first 2 chars if numeric
      WHEN SUBSTRING(name, 1, 2) ~ '^\d+$' THEN SUBSTRING(name, 1, 2)
      ELSE NULL
    END
  )
WHERE grade IS NULL;
